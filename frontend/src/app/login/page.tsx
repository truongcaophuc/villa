...
import { authLogin, apiPost } from "@/lib/backend";
      ...
      <div className="flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-sm space-y-6 text-center">
          ...
          <div className="text-sm">
            {mode === "login" ? (
              <button className="underline" onClick={() => setMode("register")}>Chưa có tài khoản? Đăng ký</button>
            ) : (
              <button className="underline" onClick={() => setMode("login")}>Đã có tài khoản? Đăng nhập</button>
            )}
          </div>
          <div className="pt-2">
            <Button
            ...
            >
              <img src="/icons/google.svg" alt="Google icon" className="w-5 h-5 mr-2" /> Đăng nhập với Google
            </Button>
          </div>
        </div>
      </div>
