...
export default async function BlogPage({
  ...
  const locale = (c.get("NEXT_LOCALE")?.value || "vi").startsWith("en")
    ? "en"
    : "vi";
    ...
    <div className="container mx-auto px-6 py-10">
      ...
      {items.length === 0 ? (
        <div className="text-center text-muted-foreground py-10">
          {t("no_blog_posts")} {/* Bạn có thể định nghĩa key này trong i18n */}
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((p) => {
              const { id, title, slug, excerpt, featured_image } = p;
                ...
                >
                  <Link href={`/blog/${slug}`} className="block">
                    <Image
                      src={
                        featured_image
                          ? resolveStorageUrl(featured_image)
                          : "/placeholder.svg"
                      }
                      alt={title}
                      width={600}
